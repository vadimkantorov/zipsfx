...
FILE* zipsfx_fopen(struct zipsfx_index* index, const char filename[], const char mode[])
{
    ...
    for(size_t i = 0; i < index->files_num; i++)
    {
        if(0 == strcmp(index->filenames + filenames_start, filename))
        {
            ...
            if(index->mmapsize != 0)
                return fmemopen((char*)index->fileptr + offset, size, "rb");
            else
            {
                FILE* f = fmemopen(NULL, size, "rb+");
                fseek((FILE*)index->fileptr, offset, SEEK_SET);
                char buf[8192];
                while(size > 0)
                {
                    size_t len = fread(buf, 1, sizeof(buf) <= size ? sizeof(buf) : size, (FILE*)index->fileptr);
                    fwrite(buf, 1, len, f);
                    size -= len;
                }
                fseek(f, 0, SEEK_SET);
...
{
    if(index->mmapsize != 0)
        munmap(index->fileptr, index->mmapsize);
    else
        fclose((FILE*)index->fileptr);
}
int zipsfx_index_build(struct zipsfx_index* index, const char filename[], int use_mmap)
{
    ...
    if(use_mmap)
    {
        ...
        index->fileptr = mmap(NULL, index->mmapsize, PROT_READ, MAP_PRIVATE, fd, 0);
        close(fd);
    }
    else
    {
        index->mmapsize = 0;
        index->fileptr = fopen(filename, "rb");
    }
    ...
    for(;;)
    {
        ...
        if(filetype == AE_IFREG && archive_entry_size_is_set(entry) != 0 && last_file_buff != NULL && last_file_buff <= firstblock_buff && firstblock_buff < last_file_buff + last_file_block_size)
        {
            ...
            index->offsets[index->files_num] = entry_byte_offset;
            index->sizes[index->files_num] = entry_byte_size;
